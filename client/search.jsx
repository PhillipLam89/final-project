...
class Search extends React.Component {
  ...
  handleBackClick() {
    this.setState({ userInput: '', hotelList: [], searchButtonClicked: '', hotelThumbnails: [], isLoading: false });
  }
  ...
  handleSearchClick(event) {
    ...
    for (let i = 0; i < numbers.length; i++) {
      if (arr[0] === '' || arr[0].includes(numbers[i])) return;
    }
      ...
      .then(data => {
        ...
        fetch(`https://hotels4.p.rapidapi.com/properties/list?destinationId=${cityId}&pageNumber=1&checkIn=2020-02-08&checkOut=2020-02-15&pageSize=25&adults1=1&currency=USD&starRatings=5&locale=en_US&sortOrder=PRICE&guestRatingMin=8`, {
          ...
          .then(data => {
            ...
            this.setState({ userInput: this.state.userInput, hotelList: this.state.hotelList, searchButtonClicked: 'hidden', hotelThumbnails: this.state.hotelThumbnails, isLoading: false });
          })
    ...
    const displayedList = hotelList.map((hotelName, idx) => {
      return (
        <div key={idx + 1} className="hotel-display-div">
          ...
          <img key={idx + hotelName} src={this.state.hotelThumbnails[idx]} className="hotel-img"></img>
        </div>
      );
    });
    return (
      <div>
        <div className={this.state.searchButtonClicked === '' ? 'hidden' : '' + ' arrow-div'}>
        ...
        <main className={this.state.searchButtonClicked === '' ? 'hidden' : ''}>
          <div className="header"><h1>{input.toUpperCase() + ' 5 STAR HOTELS:'}</h1></div>
          <div className="hotel-list">{displayedList}</div>
        </main>

        <div className={this.state.searchButtonClicked}>
          ...
          <header className={this.state.searchButtonClicked}>
            <h6>*We Only Display 5-Star Hotels Currently...</h6>
            <nav className="navbar navbar-light bg-light">
              <form onSubmit={this.handleSearchClick} className="form-inline">
                <input value={this.state.userInput} onChange={this.handleUserInput} className="form-control mr-sm-2" type="search" placeholder="city (worldwide)" aria-label="Search"></input>
